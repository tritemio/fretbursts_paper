...
After computation, one can plot the empirical standard deviation ($s_E$) of sub-burst FRET efficiencies over the whole burst with the respect to each burst's FRET efficiency as well as the expected standard deviation of a shot-noise limited distribution according to~(eq.~\ref{eq:binom_std}) as shown in the right panels of figure ~\ref{fig:bva_static} & ~\ref{fig:bva_dynamic}.
