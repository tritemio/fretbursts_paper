...
\verb|AemDex_mask_d[start:stop].sum| computes the number of photons emitted from acceptor in each sub-burst containing a constant number of consecutive photons,~\textit{n}. 
...
After computations, one can plot $s_E$ with the respect to each burst's FRET efficiency, as well as the expected standard deviation curve according to~ eq.~\ref{eq:binom_std} (red curve), as shown in the right panels of figure ~\ref{fig:bva_static} & ~\ref{fig:bva_dynamic}.
