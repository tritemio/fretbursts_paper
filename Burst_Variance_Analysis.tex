...
With FRETBursts, a user can easily get the data needed for BVA analysis. 
For start, we need two photon streams: photon during donor excitation (Dex) and acceptor photon during donor excitation (AemDex). These selections are obtained by computing photon masks as follows (see section~\ref{sec:burststimes}):
...
Finally, by plotting the 2D distribution of $s_E$ (i.e. \verb|E_sub_std|) versus the average (uncorrected) E 
