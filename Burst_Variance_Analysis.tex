...
$E$ is the FRET efficiency. (In practice, Proximity ratio is used as $E$.) Since, as mentioned, $N_a$ will follow a binomial distribution and n, we expect the standard deviation of $E$ for the sub-bursts to be distributed according to eq. ~\ref{eq:binom_std}. This is an approximation because background counts (both from sample and detector's dark counts) add additional variance that is not taken into account.
