...
%DIF ADD full_article_a936.tex   Wed Jul  6 12:57:39 2016
...
\DIFaddbegin \DIFadd{We conclude the section by surveying different methods proposed
in litterature to study FRET dynamics (section }\nameref{sec:dynamics}\DIFadd{).
}\DIFaddend As an example
of implementation of an advanced data processing technique,
section~\nameref{sec:bva} walks the reader \DIFdelbegin \DIFdel{thorough }\DIFdelend \DIFaddbegin \DIFadd{through }\DIFaddend implementing
...
graphic library\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{matplotlib}}%DIFAUXCMD
}\DIFaddend ) produce publication quality plots in a wide range of formats.
...
\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(}\href{\DIFadd{http://nbviewer.jupyter.org/github/tritemio/FRETBursts_notebooks/blob/master/notebooks/Example%DIF > 20-%20Burst%20Variance%20Analysis.ipynb}{BVA link},
}\href{\DIFadd{http://nbviewer.jupyter.org/github/tritemio/FRETBursts_notebooks/blob/master/notebooks/Example%DIF > 20-%202CDE%20Method.ipynb}{2CDE link}).
}\DIFaddend 
...
timescales, the distribution departs from \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend exponential due to the contribution
...
during donor or acceptor excitation \DIFaddbegin \DIFadd{periods }\DIFaddend (table~\ref{tab:data_n}).
...
(by default they are \DIFaddbegin \DIFadd{set to }\DIFaddend 0).
There is also a closely related attribute named \verb|nda| for donor photons \DIFaddbegin \DIFadd{detected
}\DIFaddend during acceptor excitation. \verb|nda| is normally neglected as it only contains
background.
...
the burst search should use all photons\DIFdelbegin \DIFdel{(i.e. APBS). In some }\DIFdelend \DIFaddbegin \DIFadd{, the so called
...
\DIFaddbegin \DIFadd{For additional details on burst search refer to the documentation
(}\href{http://fretbursts.readthedocs.io/en/latest/burstsearch.html}{link}\DIFadd{).
}\DIFaddend 

After burst search, it is necessary to \DIFaddbegin \DIFadd{further }\DIFaddend select
...
\DIFdelbegin \DIFdel{A simple }\DIFdelend \DIFaddbegin \DIFadd{An alternative method to apply $\gamma$ correction consist in
discarding a constant fraction of photons chosen randomly from either 
the D\textsubscript{em} or A\textsubscript{em} photon stream~\mbox{%DIFAUXCMD
...
\DIFadd{A simple }\DIFaddend way to mitigate the dependence of the FRET distribution on
...
\subsection*{Loading \DIFdelbegin \DIFdel{the }\DIFdelend Data}
\label{sec:dataload}
...
the alternation period for a smFRET measurement \DIFaddbeginFL \DIFaddFL{(in timestamp clock unit)}\DIFaddendFL . 
(b) ns-ALEX TCSPC nanotime histogram for a smFRET measurement \DIFaddbeginFL \DIFaddFL{(in TDC or TAC bin unit)}\DIFaddendFL .
...
\DIFaddbegin \noindent \DIFaddend where \verb|D_ON| and \verb|A_ON| are \DIFdelbegin \DIFdel{tuples (}\DIFdelend pairs of numbers \DIFaddbegin \DIFadd{(tuples in python}\DIFaddend ) representing
...
\begin{lstlisting}
\DIFadd{d.calc_bg(bg.exp_fit, time_s=30, tail_min_us='auto')
}\end{lstlisting}
...
\DIFadd{First, we show how to estimate the background }\DIFaddend every 30~s, 
...
\DIFdelbegin \DIFdel{for the }\DIFdelend \DIFaddbegin \DIFadd{(the same for }\DIFaddend all photon streams\DIFdelbegin \DIFdel{, the corresponding command is}\DIFdelend \DIFaddbegin \DIFadd{)}\DIFaddend :
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{However, the }\DIFaddend recommended approach is \DIFdelbegin \DIFdel{however automating the choice of threshold }\DIFdelend \DIFaddbegin \DIFadd{to choose the threshold automatically }\DIFaddend using
\verb|tail_min_us='auto'|\DIFdelbegin \DIFdel{using }\DIFdelend \DIFaddbegin \DIFadd{. This approach uses }\DIFaddend an heuristic algorithm \DIFdelbegin \DIFdel{which is described in }\DIFdelend \DIFaddbegin \DIFadd{described in the
}\DIFaddend \textit{Background estimation} section of the μs-ALEX tutorial
...
Finally, it is possible to use a \DIFdelbegin \DIFdel{slower but rigorous approach for finding the optimal
threshold}\DIFdelend \DIFaddbegin \DIFadd{rigorous but slower approach to find an optimal
threshold, }\DIFaddend as described in~\nameref{sec:bg_opt_th}. % SI_link
...
This command \DIFdelbegin \DIFdel{reflects }\DIFdelend \DIFaddbegin \DIFadd{illustrates }\DIFaddend the general form of \DIFaddbegin \DIFadd{a }\DIFaddend plotting commands in FRETBursts\DIFaddbegin \DIFadd{,
}\DIFaddend as described in~\nameref{sec:plotting}. % SI_link
...
but \DIFdelbegin \DIFdel{can }\DIFdelend they can be adjusted if needed.
...
latter case, existing burst data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend automatically updated using the new
...
Another important parameter for defining the burst size is the $\gamma$-factor, i.e. the \DIFdelbegin \DIFdel{imbalance between the donor and the acceptor channel }\DIFdelend \DIFaddbegin \DIFadd{product of the relative quantum yield and detection efficiency of the acceptor versus donor }\DIFaddend signals. As noted in
section~\nameref{sec:burstsizeweights}, the $\gamma$-factor is
used to compensate \DIFdelbegin \DIFdel{bias for the }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend different fluorescence quantum yields \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the D and A
fluorophores as well as \DIFdelbegin \DIFdel{the }\DIFdelend different photon-detection efficiencies \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the D and A channels.
When $\gamma$ is significantly different from 1, neglecting its effect on burst size leads to
over-representing \DIFdelbegin \DIFdel{(in terms of number of bursts) }\DIFdelend one FRET population versus the others.
...
%DIFDELCMD < %%%
\DIFdelend In smFRET-ALEX experiments, \DIFdelbegin \DIFdel{in addition to one or more FRET populations, there are always
}\DIFdelend donor-only (D-only) and acceptor-only (A-only) populations \DIFaddbegin \DIFadd{can be detected in addition to the FRET population(s)}\DIFaddend .
In most cases, \DIFdelbegin \DIFdel{these additional populations are not of }\DIFdelend \DIFaddbegin \DIFadd{the D-only and A-only populations are of no }\DIFaddend interest and need to be filtered out.
...
Figure~\ref{fig:alex_jointplot_fretsel} shows the resulting \DIFdelbegin \DIFdel{pure }\DIFdelend \DIFaddbegin \DIFadd{filtered }\DIFaddend FRET
...
such as \DIFdelbegin \DIFdel{1 to 3 Gaussian
}\DIFdelend \DIFaddbegin \DIFadd{1- to 3-Gaussian }\DIFaddend peaks or 2-Gaussian connected by a \DIFdelbegin \DIFdel{``bridge''.
...
Santoso et al.~\mbox{%DIFAUXCMD
\cite{santoso_probing_2009} }%DIFAUXCMD
and Kalinin et al.~\mbox{%DIFAUXCMD
\cite{Kalinin2010}
}%DIFAUXCMD
...
(BVA) proposed by Torella et al.~\mbox{%DIFAUXCMD
\cite{Torella_2011} }%DIFAUXCMD
and
two-channel kernel density estimator (2CDE) proposed by 
Tomov et al.~\mbox{%DIFAUXCMD
...
(}\href{\DIFadd{http://nbviewer.jupyter.org/github/tritemio/FRETBursts_notebooks/blob/master/notebooks/Example%DIF > 20-%20Burst%20Variance%20Analysis.ipynb}{BVA link},
}\href{\DIFadd{http://nbviewer.jupyter.org/github/tritemio/FRETBursts_notebooks/blob/master/notebooks/Example%DIF > 20-%202CDE%20Method.ipynb}{2CDE link}).
...
(}\href{\DIFadd{http://nbviewer.jupyter.org/github/tritemio/FRETBursts_notebooks/blob/master/notebooks/Example%DIF > 20-%20Working%20with%20timestamps%20and%20bursts.ipynb}{link}).
...
An empirical variance of sub-bursts larger than the \DIFdelbegin \DIFdel{shot-noise limited }\DIFdelend \DIFaddbegin \DIFadd{shot-noise-limited }\DIFaddend value indicates
the presence of dynamics. 
\DIFdelbegin \DIFdel{Since the estimation of the sub-bursts variance is affected
...
Conversely, if the observed distribution originates from biomolecules belonging to a single \DIFdelbegin \DIFdel{specie}\DIFdelend \DIFaddbegin \DIFadd{species}\DIFaddend , 
