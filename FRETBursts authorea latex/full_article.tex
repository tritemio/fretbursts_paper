...
(resulting a distribution of Förster radiuses)
or actual conformational changes (resulting in a distribution
of D-A distances)~\cite{sisamakis_accurate_2010} can cause
FRET peak broadening.
...
timescale of detectable kinetics by considering multible subsequent bursts
and computing the probability that two nearby bursts are originating from
the same molecules.
Other two related methods are Burst Variance Analysis (BVA)~\cite{Torella_2011}
and 2CDE~\cite{Tomov_2012} allows to discriminate between static heterogeneity
and dynamics by extracting information from the photon distribution
within each burst. 
...
However, motivated users can implemement their favorite method 
...
coding new methods using FRETBursts to share their contributions and
to integrate their work in FRETBursts for benefit of the entire community.
The next section, takes the BVA method as an example to show how to perform
low-level manipulation of timestamps and bursts data typically needed 
when implementing most of the previously mentioned methods. 
