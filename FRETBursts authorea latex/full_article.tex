...
discard a fixed fraction of photons from either the DexDem or DexAem
photon stream as proposed by~\cite{tsukanov_detailed_2013}. This 
simple and elegant method transforms the measurement data in order to
achieve $\gamma=1$, overcoming issues of selection bias between populations. 
The only drawback of this method is that, by discarding a fraction of photons,
it leads to information loss and therefore to potentially decreased sensitivity 
and/or accuracy.
...
As a start, we show how to estimate the background every 30~s, 
using a fixed inter-photon delay threshold of 2~ms 
(the same for all the photon streams):
...
or poorly sealed observation chamber (we strongly recommend using a sealed 
...
quantitied a burst "estimator" 
(called FRET-2CDE) is derived. To use the 2CDE method, a user can
simply plot the 2-D histogram of E versus FRET-2CDE and detect the
occurrence of dynamics by assessing the vertical position of 
the various populations (populations centered around FRET-2CDE=10 have
